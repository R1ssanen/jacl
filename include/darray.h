...
/* to help differentiate dynamic arrays
...
#define DARRAY_ASSERT(expr, msg)                                                                   \
    do {                                                                                           \
        if (!(expr)) {                                                                             \
            fprintf(                                                                               \
                stderr, "darray runtime assert at %s, line %d: %s\n", __func__, __LINE__, msg      \
            );                                                                                     \
            abort();                                                                               \
        }                                                                                          \
    } while (0)
...
#ifndef NDEBUG
#    define DARRAY_ASSERT_DEBUG(expr, msg) DARRAY_ASSERT(expr, msg)
#else
#    define DARRAY_ASSERT_DEBUG() ((void)0)
#endif
...
DARRAY_INLINE size_t darray_fetch_field_(const size_t* block, int field) {
    return *(block - field - 1);
}

DARRAY_INLINE void darray_set_field_(size_t* block, int field, size_t value) {
    *(block - field - 1) = value;
}

#define darray_stride(array)   darray_fetch_field_((const size_t*)(array), DARRAY_TAG_STRIDE_)

#define darray_count(array)    darray_fetch_field_((const size_t*)(array), DARRAY_TAG_COUNT_)

#define darray_capacity(array) darray_fetch_field_((const size_t*)(array), DARRAY_TAG_CAPACITY_)
...
void* darray_new_impl(size_t stride, size_t count) {
    DARRAY_ASSERT(stride != 0, "cannot create array with stride of 0.");

    size_t* head = (size_t*)DARRAY_ALLOC(stride * (count + 1) + DARRAY_HEADER_STRIDE_);
    DARRAY_ASSERT(head != NULL, "no memory to allocate array.");
    ...
    darray_set_field_(block, DARRAY_TAG_CAPACITY_, (count + 1) * DARRAY_GROWTH_FACTOR + 0.5f);
    return block;
}

DARRAY_INLINE void darray_free_impl(size_t* block) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");

    size_t* head = block - DARRAY_ENUM_END_;
    DARRAY_DEALLOC(head);
}

DARRAY_INLINE void* darray_resize_impl(size_t* block, size_t new_capacity) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
    ...
    DARRAY_ASSERT(new_head != NULL, "no memory to reallocate array.");
...
DARRAY_INLINE void* darray_grow_impl(size_t* block, unsigned amount) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
    ...
    if (grown_size >= capacity) {
        size_t new_capacity = amount * capacity * DARRAY_GROWTH_FACTOR + 0.5f;
...
DARRAY_INLINE void darray_pop_impl(size_t* block) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
    darray_set_field_(block, DARRAY_TAG_COUNT_, darray_count(block) - 1);
}

// move all elements above 'index' up 'amount' places
DARRAY_INLINE void* darray_shift_up_impl(size_t* block, size_t index, size_t amount) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
...
DARRAY_INLINE void darray_shift_down_impl(size_t* block, size_t index, size_t amount) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
...
DARRAY_INLINE void* darray_insert_impl(size_t* block, size_t index, const void* value) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
    DARRAY_ASSERT_DEBUG(value != NULL, "value argument is null.");

    size_t count = darray_count(block);
    DARRAY_ASSERT_DEBUG(index < count, "array index out of bounds.");
...
DARRAY_INLINE void darray_remove_impl(size_t* block, size_t index, size_t amount) {
    DARRAY_ASSERT_DEBUG(block != NULL, "array argument is null.");
...
#define darray_new_reserved(type, count) ((type*)(darray_new_impl(sizeof(type), count)))

#define darray_free(array)               darray_free_impl((size_t*)(array))

#define darray_clear(array)              darray_set_field_((size_t*)(array), DARRAY_TAG_COUNT_, 0)

#define darray_last(array)               ((array) + darray_count(array) - 1)

#define darray_resize(array, count)                                                                \
    do { (array) = darray_resize_impl((size_t*)(array), count); } while (0)

#define darray_shrink(array)         darray_resize(array, darray_count(array))

#define darray_reserve(array, count) darray_resize(array, darray_count(array) + (count))
...
#define darray_insert(array, index, value)                                                         \
    do { (array) = darray_insert_impl((size_t*)(array), index, value); } while (0)

#define darray_remove(array, index)             darray_remove_impl((size_t*)(array), index, 1)

#define darray_remove_span(array, index, count) darray_remove_impl((size_t*)(array), index, count)

#define darray_foreach(array, type, it)         for (type* it = (array); it <= darray_last(array); ++it)
