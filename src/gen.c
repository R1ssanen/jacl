...
static void emit(generator *, enum section, const char *, ...);

static void emit_impl(generator *gen, enum section section_, size_t indent, const char *fmt, va_list args)
{
    char buffer[256];
...
static void emit(generator *gen, enum section section_, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
static void emit_noindent(generator *gen, enum section section_, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
static void push(generator *gen, const char *value)
{
    emit(gen, SECTION_TEXT, "push %s\n", value);
}
...
static void generate_expression(generator *, const jac_ast_expression *, jac_ast_variable_definition **);

static void generate_statement_expression_call(generator *gen, const jac_ast_function_call *call,
...
static void generate_variable_definition(generator *gen, const jac_ast_variable_definition *var_def,
                                         jac_ast_variable_definition **variables)
{
...
static void generate_assignment(generator *gen, const jac_ast_assignment *assignment,
                                jac_ast_variable_definition **variables)
{
...
static void generate_statement_expression(generator *gen, const jac_ast_expression_statement *expr_statement,
                                          jac_ast_variable_definition **variables)
{
...
static void generate_identifier(generator *gen, const jac_token *identifier, jac_ast_variable_definition **variables)
{
    darray_foreach(*variables, jac_ast_variable_definition, variable)
    {
...
static void generate_reference(generator *gen, const jac_token *addrof, jac_ast_variable_definition **variables_p)
{
...
static void generate_expression(generator *gen, const jac_ast_expression *expression,
                                jac_ast_variable_definition **variables)
{
    ...
    case JAC_AST_EXPRESSION_LITERAL:
        generate_literal(gen, expression->opt.literal);
        break;
...
static void generate_statement_return(generator *gen, const jac_ast_statement_return *ret,
                                      jac_ast_variable_definition **variables)
{
...
static void generate_block(generator *gen, const jac_ast_block *block, jac_ast_variable_definition **variables)
{
    ...
    darray_foreach(block->statements, jac_ast_scope_statement, statement)
    {
        ...
        case JAC_AST_STATEMENT_RETURN:
            ...
            break;
        case JAC_AST_BLOCK:
            generate_block(gen, statement->opt.block, variables);
            break;
        case JAC_AST_EXPRESSION_STATEMENT:
...
static void generate_unit_statement(generator *gen, const jac_ast_unit_statement *statement)
{
    ...
    case JAC_AST_EXTERN_BLOCK:
        generate_extern_block(gen, statement->opt.extern_block);
        break;
...
bool jac_generate_unit(jac_ast_unit *unit, char **source_out)
{
    ...
    darray_foreach(unit->statements, jac_ast_unit_statement, statement) generate_unit_statement(&gen, statement);
