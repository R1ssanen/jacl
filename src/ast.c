...
// void* darray_alloc(size_t c) { return malloc(c); }

// void* darray_realloc(void* p, size_t c) {
//     printf("resize: %lu\n", c);
//     return realloc(p, c);
// }

// void darray_dealloc(void* p) { free(p); }
...
static bool
parse_statement_expression_call(parser* parser, bool required, jac_ast_function_call* call) {
    ...
    if (!expect(parser, JAC_TOKEN_LPAREN)) {
        if (required) jac_print_diagnostic(parser->c, "expected '(', got '%s'.", parser->c->value);
        return false;
    }
            ...
            if (!parse_expression(parser, required, &expression)) {
                if (required) {
                    jac_print_diagnostic(
                        parser->c, "expected argument expression, got '%s'.", parser->c->value
                    );
                }
                return false;
            }
...
static bool
parse_variable_definition(parser* parser, bool required, jac_ast_variable_definition* var_def) {

    if (!parse_variable_declaration(parser, required, &var_def->declaration)) return false;
    ...
    jac_ast_expression expression;
    if (!parse_expression(parser, required, &expression)) return false;
...
static bool parse_statement_expression(
    parser* parser, bool required, jac_ast_expression_statement* expr_statement
) {
    ...
    case JAC_TOKEN_IDENTIFIER: {
        ...
        if (parser->c[1].kind == JAC_TOKEN_LPAREN) {
            ...
            if (!parse_statement_expression_call(parser, required, &call)) return false;
            ...
            *expr_statement->opt.call = call;
            ...
            if (!parse_assignment(parser, required, &assignment)) return false;
    ...
    default: {
        ...
        if (parse_variable_definition(parser, required, &var_def)) {
            ...
            expr_statement->opt.var_def =
                jac_arena_alloc(parser->arena, jac_ast_variable_definition);
            *expr_statement->opt.var_def = var_def;
            return true;
        }

        if (required) jac_print_diagnostic(parser->c, "invalid statement expression.");
        return false;
    }
    }
}
...
static bool parse_expression(parser* parser, bool required, jac_ast_expression* expression) {
    ...
    case JAC_TOKEN_IDENTIFIER: {
        if (parser->c[1].kind == JAC_TOKEN_LPAREN) {
            ...
            if (!parse_statement_expression(parser, required, &expr_statement)) return false;

            expression->holds = JAC_AST_EXPRESSION_STATEMENT;
            expression->opt.expr_statement =
                jac_arena_alloc(parser->arena, jac_ast_expression_statement);
    ...
    default:
        if (required) jac_print_diagnostic(parser->c, "invalid expression.");
        return false;
    }
}
...
static bool
parse_scope_statement(parser* parser, bool required, jac_ast_scope_statement* statement) {

    switch (parser->c->kind) {

    case JAC_TOKEN_RETURN: {
        jac_ast_statement_return ret;
        if (!parse_statement_return(parser, required, &ret)) return false;
    ...
    case JAC_TOKEN_LBRACE: {
        if (!parse_block(parser, required, &statement->opt.block)) return false;
    ...
    default: {
        ...
        if (required) jac_print_diagnostic(parser->c, "invalid scope statement.", parser->c->value);
        return false;
    }
    }
}
...
static bool parse_block(parser* parser, bool required, jac_ast_block* block) {
    ...
    *block = darray_new(jac_ast_scope_statement);
    ...
    while (!is_eof(parser) && (parser->c->kind != JAC_TOKEN_RBRACE)) {
        ...
        if (!parse_scope_statement(parser, required, &statement)) return false;
...
static bool parse_type(parser* parser, bool required, jac_type* type) {
    ...
    case JAC_TOKEN_UINT8: type->kind = JAC_TYPE_UINT8; break;
    ...
    default:
        if (required) jac_print_diagnostic(parser->c, "expected type, got '%s'.", parser->c->value);
        return false;
    }
...
static bool parse_variable_declaration(parser* parser, bool required, jac_symbol* declaration) {
    ...
    if (!parse_type(parser, required, &declaration->type)) return false;
...
static bool parse_function_header(parser* parser, bool required, jac_function* header) {
    ...
    if (expect(parser, JAC_TOKEN_LPAREN)) {
        ...
        if (parse_variable_declaration(parser, false, &arg)) {
            ...
            while (expect(parser, JAC_TOKEN_COMMA)) {
                if (!parse_variable_declaration(parser, required, &arg)) return false;
                darray_push(header->args, arg);
            }
        }
...
static bool parse_extern_block(parser* parser, bool required, jac_ast_extern_block* block) {
    ...
    *block = darray_new(jac_function);
    while ((parser->c->kind != JAC_TOKEN_EOF) && (parser->c->kind != JAC_TOKEN_RBRACE)) {
        jac_function header;
        if (!parse_function_header(parser, true, &header)) return false;
...
static bool
parse_function_definition(parser* parser, bool required, jac_ast_function_definition* func_def) {
    if (!parse_function_header(parser, required, &func_def->header)) return false;
    if (!parse_block(parser, true, &func_def->block)) return false;
    return true;
}
...
static bool parse_unit_statement(parser* parser, jac_ast_unit_statement* statement) {
    ...
    case JAC_TOKEN_FUNC: {
        ...
        if (!parse_function_definition(parser, true, &func_def)) return false;
    ...
    case JAC_TOKEN_EXTERN: {
        if (!parse_extern_block(parser, true, &statement->opt.extern_block)) return false;
        statement->holds = JAC_AST_EXTERN_BLOCK;
        return true;
    }

    default: jac_print_diagnostic(parser->c, "invalid unit statement."); return false;
    }
}
...
static void free_statement_expression(jac_ast_expression_statement* expr_statement) {
    ...
    case JAC_AST_FUNCTION_CALL: free_statement_expression_call(expr_statement->opt.call); break;
    case JAC_AST_VARIABLE_DEFINITION: free_variable_definition(expr_statement->opt.var_def); break;
    case JAC_AST_ASSIGNMENT: free_assignment(expr_statement->opt.assignment); break;
...
static void free_expression(jac_ast_expression* expression) {
    ...
    case JAC_AST_EXPRESSION_STATEMENT:
        ...
        break;
    case JAC_AST_EXPRESSION_LITERAL: break;
    case JAC_AST_EXPRESSION_IDENTIFIER: break;
    case JAC_AST_EXPRESSION_ADDROF: break;
...
static void free_return(jac_ast_statement_return* ret) {
    if (ret->expression) free_expression(ret->expression);
}
...
static void free_block(jac_ast_block block) {
    ...
    darray_foreach(block, jac_ast_scope_statement, statement) {
        ...
        case JAC_AST_STATEMENT_RETURN: free_return(statement->opt.ret); break;
        case JAC_AST_EXPRESSION_STATEMENT:
...
static void free_function_header(jac_function* header) { darray_free(header->args); }
...
void jac_free_unit(jac_ast_unit unit) {
    ...
    darray_foreach(unit, jac_ast_unit_statement, statement) {
        switch (statement->holds) {
        case JAC_AST_FUNCTION_DEFINITION: free_function_definition(statement->opt.func_def); break;
        case JAC_AST_EXTERN_BLOCK: free_extern_block(statement->opt.extern_block); break;
...
static void
print_statement_expression_call(const jac_ast_function_call* call, int level, int indent) {
...
static void
print_variable_definition(const jac_ast_variable_definition* var_def, int level, int indent) {
...
static void print_statement_expression(
    const jac_ast_expression_statement* expr_statement, int level, int indent
) {
...
static void print_statement_return(const jac_ast_statement_return* ret, int level, int indent) {
    ...
    if (ret->expression) print_expression(ret->expression, level + indent, indent);
    else
...
static void print_block(const jac_ast_block block, int level, int indent) {
    ...
    darray_foreach(block, jac_ast_scope_statement, statement) {
        ...
        case JAC_AST_STATEMENT_RETURN:
            ...
            break;
        case JAC_AST_BLOCK: print_block(statement->opt.block, level + indent, indent); break;
        case JAC_AST_EXPRESSION_STATEMENT:
...
static void print_type(const jac_type* type, int level, int indent) {
    ...
    switch (type->kind) {
    case JAC_TYPE_NONE: indented(level, "none"); break;
    case JAC_TYPE_CUSTOM: print_token(type->token, "custom", level); break;
    ...
    case JAC_TYPE_INT64:
    case JAC_TYPE_UINT64: print_token(type->token, "intrinsic", level); break;
...
static void print_function_header(const jac_function* header, int level, int indent) {
    ...
    darray_foreach(header->args, jac_symbol, arg)
        print_variable_declaration(arg, level + indent, indent);
}

static void print_extern_block(const jac_ast_extern_block block, int level, int indent) {
    indented(level, "extern_block");
    darray_foreach(block, jac_function, header)
        print_function_header(header, level + indent, indent);
}

static void
print_function_definition(const jac_ast_function_definition* func_def, int level, int indent) {
